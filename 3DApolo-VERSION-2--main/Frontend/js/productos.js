  ...
  data.datos.forEach((p) => {
    ...
    tbody.insertAdjacentHTML('beforeend', `
      <tr data-id="${p.id_producto}">
        ...
        <td>
          <div class="acciones">
            <button class="btn-icono" onclick="verProducto(${p.id_producto})">👁</button>
            <button class="btn-icono" onclick="abrirEditar(${p.id_producto})">✏️</button>
            <button class="btn-icono rojo" onclick="eliminarProducto(${p.id_producto},'${p.nombre}')">🗑</button>
          </div>
        </td>
      </tr>`);
  });
...
$('#form-producto').addEventListener('submit', async (e) => {
  ...
  const payload = {
    ...
    costo_prom:   tipo === 'fabricado' ? ($('#form-costo-fab').value || 0) : undefined,
...
window.agregarFilaMaterial = (idMateria = '', cantidad = '') => {
  ...
  const opts = materiasDisponibles.map(m => {
    ...
    return `<option value="${m.id_materia}"
      data-stock="${m.stock}"
      data-costo="${m.costo_prom}"
  ...
  fila.innerHTML = `
    <select class="mat-select"
      onchange="validarStockFila(this); recalcularPrecioFabricado()">
      <option value="">— Materia prima —</option>
      ${opts}
    </select>
    <input type="number" class="mat-cant" min="0.0001" step="0.0001"
      value="${cantidad || ''}" placeholder="Cantidad"
      oninput="validarCantFila(this); recalcularPrecioFabricado()">
    <button type="button" class="mat-del"
      onclick="this.closest('.fila-material').remove(); recalcularPrecioFabricado()">✕</button>`;
  lista.appendChild(fila);

  if (cantidad) {
    validarCantFila(fila.querySelector('.mat-cant'));
    recalcularPrecioFabricado();
  }
};
