...
function badge(e) {
    const m = { pendiente: ['badge-naranja', 'Pendiente'], en_proceso: ['badge-azul', 'En proceso'], completada: ['badge-verde', 'Completada'], cancelada: ['badge-rojo', 'Cancelada'] };
...
async function cargarCatalogos() {
    ...
    (d.productos || []).forEach(p => {
        ...
        o.value = p.id_producto; o.textContent = p.nombre; sel.appendChild(o);
    });
}
...
async function cargar() {
    ...
    document.getElementById('ci-proceso').textContent = datos.filter(x => x.estado === 'en_proceso').length;
    document.getElementById('ci-comp').textContent = datos.filter(x => x.estado === 'completada').length;
    document.getElementById('ci-pend').textContent = datos.filter(x => x.estado === 'pendiente').length;
    renderTabla(); renderPag();
}

function renderTabla() {
    const tb = document.getElementById('tbody');
    if (!datos.length) { tb.innerHTML = '<tr><td colspan="8" class="tbl-empty">Sin órdenes registradas</td></tr>'; return; }
    tb.innerHTML = datos.map(o => `
      <tr>
        ...
        <td>
          ...
          <button class="btn-acc rojo" title="Eliminar" onclick="eliminar(${o.id_orden})"><i data-lucide="trash-2"></i></button>
        </td>
      </tr>`).join('');
    lucide.createIcons();
}
...
function editar(id) {
    const o = datos.find(x => x.id_orden === id); if (!o) return;
    editId = id;
...
async function eliminar(id) {
    ...
    if (!d?.error) { toast('Orden eliminada'); cargar(); } else toast(d.error || 'Error', 'err');
}
...
async function guardar(e) {
    ...
    if (!d?.error) { toast(editId ? 'Orden actualizada' : 'Orden creada'); cerrar(); cargar(); }
